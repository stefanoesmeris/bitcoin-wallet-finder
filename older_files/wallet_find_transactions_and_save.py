...
def check_addresses(derivation, coin_type, label, bip_number, mnemonic):
    ...
    global found_flag
    found_flag = True
    print("Derivation: ", derivation, "Label: ", label, "Bip Number: ", bip_number)
...
def write_file(data,mnemonic):
    ...
    if found_flag:
        ...
        print("\n🛑 Resultado exportado para 'carteira_bitcoin_bad.txt'"
    else:
        df = pd.DataFrame(data)
        df.to_csv("carteira_bitcoin_good.txt", mode='a', index=False)
        print("\n✅ Resultado exportado para 'carteira_bitcoin_good.txt'")   
...
def main():
    ...
    try:
        while True:
            ...
            time.sleep(1.9)  # Sleep for half a second
            lista = seq.get_next(int(contador),int(24))
            print(lista)
            contador += 1
